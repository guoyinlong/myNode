  ...
  effects : {
     ...
     */
      *fetch({year}, {call, put}) {
      ...
      yield put({type: 'wageInfoQuery',year});
    },
    /**
     * 功能：初始化员工考核结果列表
     * 作者：陈莲
     ...
     */
      *indexTemplateQuery({year}, {call, put}) {
      ...
      if(infoRes.RetCode=='1' && infoRes.DataRows && infoRes.DataRows.length){
        ...
        message.info(year+"年整体激励报告模板未配置！")
      }
    },
    /**
     * 功能：初始化员工考核结果列表
     ...
     * 创建日期：2017-07-20
     */
    *wageInfoQuery({year}, {call, put}) {
      yield put({
        type: 'saveInfoRes',
        info:{},
        year:year
      });
      const infoRes = yield call(service.wageInfoQuery,
        {
          arg_year:year
        });
  ...
  subscriptions : {
    setup({dispatch, history}) {
      ...
      return history.listen(({pathname, query}) => {
        ...
        // if (pathname === '/humanApp/encouragement/wage') {
