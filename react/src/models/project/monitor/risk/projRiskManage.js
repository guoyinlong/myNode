...
export default {
  ...
  effects: {
    ...
    *startListSearch({}, { call, put,select }) {
      ...
      let queryData = yield select(state => state.projRiskManage);
      let postData = getPostData(queryData);
      yield put({ type:'conditionSearch', postData:postData});
      yield put({ type:'getProjNameList', postData});
    },
    ...
    *riskExport({param},{call,put}){
      let postData = {}
      postData["arg_ou"] = (param == undefined ) ? "联通软件研究院本部" : param["arg_ou"];
      // console.log(postData)
      const basicInfoData = yield call(projServices.riskTransExport, postData);
      for(let i= 0 ; i< basicInfoData.DataRows.length; i++){
        ...
        basicInfoData.DataRows[i]["跟踪进展情况"] = (basicInfoData.DataRows[i]["跟踪进展情况"] == undefined) ? "" : basicInfoData.DataRows[i]["跟踪进展情况"];
        basicInfoData.DataRows[i]["缓解措施"] = (basicInfoData.DataRows[i]["缓解措施"] == undefined) ? "" : basicInfoData.DataRows[i]["缓解措施"];
      }
      // console.log(basicInfoData)
      yield put({
        type: 'save',
        payload: {
          riskExportList: basicInfoData.DataRows
        }
      });
    },
    /**
     ...
     */
      *hideMainDeptModel({dept_name},{put,select}){
      ...
      let queryData = yield select(state => state.projRiskManage);
      console.log(queryData,"hideMainDeptModel")
      let postData = getPostData(queryData);
      yield put({ type:'conditionSearch', postData:postData});
      yield put({ type:'getProjNameList', postData:postData});
    },
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen(({ pathname, query }) => {
        if (pathname === '/projectApp/projMonitor/risk') {
