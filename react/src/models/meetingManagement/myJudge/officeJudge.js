...
export default {
  ...
  effects: {
    ...
    * isDept({record},{put}){
    ...
    //点击弹出申请单位弹出框
    * handleDeptModal({},{call, select, put}){
      yield put({
        type:'save',
        payload: {
          deptVisible:true,
    ...
    //申请单位点击确定按钮存入
    * handleDeptOk({value},{call,select,put}){
      const { deptInputs } = yield select(state => state.officeJudge);
    ...
    * onDeptChecked({value},{call,select,put}){
      const { deptInputs,deptId } = yield select(state => state.officeJudge);
      /* console.log(value);*/
    ...
    * waitDetails({},{call, select, put}){
      const { passData } = yield select(state=>state.officeJudge);
    ...
    * searchUploadFile({},{call, select, put}){
      ...
      const { topicID,submitID  }=yield select (state=>state.officeJudge);
      const recData={
    ...
    * handleAgreen({}, { call, put, select }){
      const { waitMeetingDetails,topicModl,isDept,deptId } =yield select (state => state.officeJudge);
      let recData;
      let response;
      if(isDept === ""){
        message.info('请选择是否需要会签')
      }else if(Number(isDept) === 1 && deptId.length === 0){
        message.info('请选择会签部门')
      }else{
        if(Number(topicModl) === 0){
          ...
          recData={
            ...
            arg_topic_dept_id:waitMeetingDetails.topic_dept_id,//| VARCHAR(1000)|是| 参会部门|
    ...
    * sendMessage({},{call, select,put}){
      ...
      const { waitMeetingDetails } =yield select (state => state.officeJudge);
      const recData = {
    ...
    * returnService({}, { call, put, select }){
      const { returnReason,waitMeetingDetails } = yield select (state=>state.officeJudge);
  ...
  subscriptions: {
    setup({
      ...
      return history.listen(({
        ...
        if (pathname === '/adminApp/meetManage/myJudge/officeJudge') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
