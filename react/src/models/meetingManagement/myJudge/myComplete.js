...
export default {
  ...
  effects: {
    * init({query}, {call, put}) {
      ...
      yield put({
        ...
        payload:{
          ...
          submitID:JSON.parse(query.value).submit_id,
    ...
    * waitDetails({},{call, select, put}){
      const { passData } = yield select(state=>state.myComplete);
    ...
    * searchUploadFile({},{call, select, put}){
      const { passData,submitID }=yield select (state=>state.myComplete);
      const recData={
        arg_topic_id:passData.topic_id,// VARCHAR(32), -- 议题id
  ...
  subscriptions: {
    setup({
      ...
      return history.listen(({
        ...
        if (pathname === '/adminApp/meetManage/myJudge/myComplete') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
