...
export default {
  ...
  state: {
    ...
    reletiveDiscussDisplay: '', //总经理办公会前置相关议题 内容是否显示
  ...
  effects: {
    * init({
      ...
      /* /!*console.log(query.recordValue)*!/
       console.log(typeof query.recordValue)*/
      /* console.log(JSON.parse(query.recordValue));*/
      let nameId = [];
      for (let j = 0; j < JSON.parse(query.recordValue).topic_user_name.split(",").length; j++) {
        /*console.log(res[i].topic_user_id.split(","));*/

        nameId.push(JSON.stringify({
          userid: JSON.parse(query.recordValue).topic_user_id.split(",")[j],
          username: JSON.parse(query.recordValue).topic_user_name.split(",")[j]
        }))
      }

      //console.log(JSON.parse(query.recordValue).topic_other_dept_id.split(','))
      yield put({
        type: 'save',
        payload: {
          passData: JSON.parse(query.recordValue),
          topicName: JSON.parse(query.recordValue).topic_name,
          deptId: JSON.parse(query.recordValue).topic_dept_id.split(','), //申请部门
          outDeptId: JSON.parse(query.recordValue).topic_other_dept_id.split(','), //列席部门
          applyReset: nameId,
          topic_id: "",
    ...
    * importantReason({}, {
      ...
      const {
        isDept,
        meetingTypeId
      } = yield select(state => state.topicReset);
      let recData;
      if (Number(isDept) === 0) {
        recData = {
          arg_ouid: 'e65c02c2179e11e6880d008cfa0427c4',
          arg_type_id: meetingTypeId,
        };
      } else {
        recData = {
      ...
      if (response.RetCode === '1') {
        ...
        for (let i = 0; i < res.length; i++) {
          ...
          for (let j = 0; j < res[i].topic_user_name.split(",").length; j++) {
            ...
            /*console.log(res[i].topic_user_id.split(","));*/
            nameId.push(JSON.stringify({
              userid: (res[i].topic_user_id).split(",")[j],
              username: (res[i].topic_user_name).split(",")[j]
            }))
          }

          /* console.log(nameId);*/
          if (res[i].topic_check_state === '0') {
            yield put({
              type: 'save',
              payload: {
                buttonDisplay: 'block'
          ...
          yield put({
            ...
            payload: {
              ...
              // outDept:res[i].other_dept_name.split(','),           //列席部门显示
              ...
              isDept: Number(res[i].topic_level), //申请单位  院级  分院级
              isUrgent: Number(res[i].topic_urgent), //紧急程度
              urgentReason: res[i].urgent_reason, //紧急原因及拟上会时间
              reasonSelected: {
                reason_name: res[i].topic_important_reason,
                reason_uuid: res[i].topic_important_uuid
              },
            }
          })
        }
    ...
    * handleMeetingTypeId({
      ...
      if (JSON.parse(i).type_name === "总经理办公会") {
        yield put({
          type: 'save',
          payload: {
            noStarDisplay: 'none',
    ...
    * handleDeptOk({}, {
      ...
      //const { deptInputs } = yield select(state => state.topicReset);
    ...
    * submissionTopic({}, {
      ...
      if (writeMinute > 30) {
      ...
      } else if (meetingTypeReset === '总经理办公会' && discussRadioValue === '') {
        message.info('请选择该议题是否需要党委会前置讨论选项');
      } else if (Number(radioValue) === 1 && reasonSelected === {
          reason_name: ''
        }) {
        ...
        if (outDeptId.length > 5) {
          ...
          const recData = {
            ...
            arg_topic_dept_id: deptId.join(), // 申请部门id，id可以多个用逗号隔开 |
            arg_topic_user_id: m.join(), //| 是 | 议题汇报人id，可以多个，用逗号隔开|
            arg_topic_user_name: n.join(), //deptId是 | 议题汇报人名称，可以多个，用逗号隔开 |
    ...
    * saveTopic({}, {
      ...
      if (writeMinute > 30) {
        ...
        message.info('请上传上会材料');
      } else if (writeMinute === '' || topicName === '' || deptId === '' || applyReset === '' || outDeptId === '') {
        message.info('有必填项没填');
      } else {
        if (outDeptId.length > 5) {
          message.info('您已经选择了5个以上列席部门，建议不超过5个');
          const recData = {
            ...
            arg_topic_check_state: passData.topic_check_state, //VARCHAR(1),-- 议题状态
            arg_topic_check_state_desc: passData.topic_check_state_desc, //VARCHAR(100),-- 议题状态描述
            arg_upload_info: JSON.stringify(file), //JSON.stringify(tableUploadFile.filter(i=>!i.hasOwnProperty('upload_id'))),
  ...
  subscriptions: {
    setup({
      ...
      return history.listen(({
        ...
        if (pathname === '/adminApp/meetManage/topicApply/topicReset') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
