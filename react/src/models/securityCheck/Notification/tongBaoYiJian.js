...
export default {
  ...
  effects: {
    ...
    *tongbaoyijian({query}, {call, put}){
      ...
      const response = yield call(myserver.Tongbaoyijian, recData);
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/securityCheck/Notification/tongBaoYiJian') { //此处监听的是连接的地址
          dispatch({
            type: 'tongbaoyijian',
            query
          });
