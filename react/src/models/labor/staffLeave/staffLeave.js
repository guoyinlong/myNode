...
export default {
  ...
  effects: {
     ...
     * @param put 返回reducer
     */
    * staffLeaveSearch_default({}, {call, put}) {
      //从服务获取OU列表
      let postData_getOU = {};
    ...
    * getPost({arg_param}, {call, put}) {
     ...
     * @param put 返回reducer
     */
    * staffLeaveSearch({arg_param}, {call, put}) {
    ...
    * staffLeave({arg_param}, {call, put}) {
      ...
      const {RetCode} = yield call(hrService.staffLeave, arg_param);
      if (RetCode === '1') {
  ...
  subscriptions:{
    setup({ dispatch, history }) {
      return history.listen(({ pathname, query }) => {
        if (pathname === '/humanApp/hr/staffLeave') {
          dispatch({ type: 'staffLeaveSearch_default',query });
        }
      });
