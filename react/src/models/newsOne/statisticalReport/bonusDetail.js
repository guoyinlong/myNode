    ...
    reducers: { // 刷新数据
  	...
  	effects: {
        ...
        *queryUserInfo({}, {call, put, select}){
			...
			const {approval_id,difference} = yield select(v =>v.bonusDetail)
			if(difference=="审核"){
					let recData = {
            ...
            let detailData = yield call(myServices.queryBonusItem, recData);
		 ...
		 * gaojianhuanjie({}, { call, put ,select}) {
			const {approval_id,tableId} = yield select(v =>v.bonusDetail)
			let recData = {
				id:tableId?tableId:approval_id
			};
			const response = yield call(myServices.queryBonusExamineItem, recData);
			if (response.retCode === '1') {
		 ...
		 * onAgree({},{call, put,select}){
			const {taskid,auditProcess,isYearNews}= yield select (state =>state.bonusDetail);
		  ...
		  * handle({},{call, put,select}){
			const {taskid,tuihuiValue}= yield select (v =>v.bonusDetail);
			if(tuihuiValue==""){
	...
	subscriptions: {
		setup({dispatch, history}) {
			return history.listen(({pathname, query}) => {
				if(pathname === '/adminApp/newsOne/statisticalReport/bonusDetail'){
					dispatch({
						type: 'init',
								query
                        });
