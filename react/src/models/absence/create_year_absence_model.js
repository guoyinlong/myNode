...
export default {
  namespace: 'create_year_absence_model',
  state: {
    circulationType: '',
    approvType: '',
    tableDataList: [],
  ...
  effects: {
    ...
    * postDatatoCreate({ query }, { call, put }) {
      ...
      if (query.saveViewControl === "none") {
        ...
        if (personData.RetCode === '1') {
          ...
          yield put({
            type: 'save',
            payload: {
              defaultDataSource1: query.circulationType === "年假申请" ? [] : resultList,
        ...
        if (personData1.RetCode === '1') {
          ...
          yield put({
            type: 'save',
            payload: {
              defaultDataSource2: query.circulationType === "年假申请" ? [] : resultList1,
            }
          })
        } else {
          message.error("没有请假人员信息");
    ...
    //保存信息
    * absenceApprovalSave({ basicInfoData, absenceDataList, yearDataInfo, absence_apply_id, resolve }, { call }) {
      let paramCheckSave = {}
      ...
      paramCheck1["arg_absence_type"] = '2';
      ...
      if (saveBasicInfo.RetCode === '1') {
        ...
        if (absenceDataListResult.RetCode !== '1') {
          ...
          const insertYearBreakUseResylt = yield call(absenceService.insertYearBreakUse, yearDataInfo);
          if (insertYearBreakUseResylt.RetCode !== '1') {
            /* 回滚功能:工作流 */
            yield call(absenceService.absenceInfoDel, postDataDeleteDateBase);
            message.error('提交失败');
            rollbackFlag = 1;
    ...
    //提交信息
    * absenceApprovalSubmit({ basicInfoData, absenceDataList, yearDataInfo, approvalData, approvalDataNext, absence_role_info, absence_apply_id, resolve }, { call }) {

      if (absence_role_info[0].absence_role == 'in_team_person') {
        /*调用工作流开始服务，传入 启动工作流标识 返回proc_inst_id，post_id_next*/
        let param = {
          //离职申请启动工作流标识
          start_type: 'absence_year_team_person',
        };
        ...
        let param = {
          //离职申请启动工作流标识
          start_type: 'absence_year_noteam_person',
        };
        const absenceFlowStartResult = yield call(absenceService.yearAbsenceInTeamApplyFlowStart, param);
        let absenceFlowStartList = [];
        if (absenceFlowStartResult.RetCode === '1') {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen(({ pathname, query }) => {
        if (pathname === '/humanApp/absence/absenceIndex/create_year_absence') {
