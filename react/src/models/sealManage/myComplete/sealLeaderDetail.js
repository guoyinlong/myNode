...
export default {
  ...
  effects: {
    ...
    * taskInfoSearch({}, {select,call, put}){
      const {passData} = yield select(state=>state.sealLeaderDetail);
      let recData={
    ...
    * downPage({}, {select}){
      ...
      const {passData} = yield select(state=>state.sealLeaderDetail);
      window.open("/microservice/allmanagementofseal/sealOfUse/LeaderSealWord?arg_form_uuid=" + passData.form_uuid);
    },
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/myJudge/sealLeaderDetail'||pathname === '/adminApp/sealManage/sealPersonalQuery/sealLeaderDetail'||pathname === '/adminApp/sealManage/managerSealQuery/sealLeaderDetail') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
