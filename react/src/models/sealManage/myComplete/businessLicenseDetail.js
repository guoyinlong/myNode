...
export default {
  namespace: 'businessLicenseDetail',
  state: {
    dataInfo:[], //详情数据
  ...
  effects: {
    ...
    * taskInfoSearch({}, {select,call, put}){
      const {passData} = yield select(state=>state.businessLicenseDetail);
      let recData={
    ...
    * downPage({}, {select}) {
      ...
      const {passData} = yield select(state => state.businessLicenseDetail);
      window.open("/microservice/allmanagementofseal/sealOfUse/PermitWord?arg_form_uuid=" + passData.form_uuid);
    },
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/myJudge/businessLicenseDetail'||pathname === '/adminApp/sealManage/sealPersonalQuery/businessLicenseDetail'||pathname === '/adminApp/sealManage/managerSealQuery/businessLicenseDetail') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
