...
export default {
  ...
  effects: {
    ...
    * taskInfoSearch({}, {select,call, put}){
      const {passData} = yield select(state=>state.borrowLeaderDetail);
      let recData={
    ...
    * downPage({}, {select}) {
      ...
      const {passData} = yield select(state => state.borrowLeaderDetail);
      window.open("/microservice/allmanagementofseal/sealOfBorrowAndCarve/ExportSealWord/LeaderSealBorrowWord?form_uuid=" + passData.form_uuid);
    },
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/myJudge/borrowLeaderDetail'||pathname === '/adminApp/sealManage/sealPersonalQuery/borrowLeaderDetail'||pathname === '/adminApp/sealManage/managerSealQuery/borrowLeaderDetail') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
