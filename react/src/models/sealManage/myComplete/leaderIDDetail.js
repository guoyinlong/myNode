...
export default {
  ...
  effects: {
    ...
    * taskInfoSearch({}, {select,call, put}){
      const {passData} = yield select(state=>state.leaderIDDetail);
      let recData={
    ...
    * downPage({}, {select}) {
      ...
      const {passData} = yield select(state => state.leaderIDDetail);
      window.open("/microservice/allmanagementofseal/sealOfUse/LeaderCardWord?arg_form_uuid=" + passData.form_uuid);
    },
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/myJudge/leaderIDDetail'||pathname === '/adminApp/sealManage/sealPersonalQuery/leaderIDDetail'||pathname === '/adminApp/sealManage/managerSealQuery/leaderIDDetail') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
