...
export default {
  ...
  effects: {
    ...
    * taskInfoSearch({}, {select,call, put}){
      const {passData} = yield select(state=>state.borrowSealDetail);
      let recData={
    ...
    * downPage({}, {select}) {
      ...
      const {passData} = yield select(state => state.borrowSealDetail);
      window.open("/microservice/allmanagementofseal/sealOfBorrowAndCarve/ExportSealWord/OfficialSealWord?form_uuid=" + passData.form_uuid);
    },
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/myJudge/borrowSealDetail'||pathname === '/adminApp/sealManage/sealPersonalQuery/borrowSealDetail'||pathname === '/adminApp/sealManage/managerSealQuery/borrowSealDetail') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
