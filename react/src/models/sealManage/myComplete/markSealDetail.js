...
export default {
  ...
  effects: {
    ...
    * taskInfoSearch({}, {select,call, put}){
      const {passData} = yield select(state=>state.markSealDetail);
      let recData={
    ...
    * downPage({}, {select}) {
      ...
      const {passData} = yield select(state => state.markSealDetail);
      window.open("/microservice/allmanagementofseal/sealOfBorrowAndCarve/ExportSealWord/carveWord?form_uuid=" + passData.form_uuid);
    },
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/myJudge/markSealDetail'||pathname === '/adminApp/sealManage/sealPersonalQuery/markSealDetail'||pathname === '/adminApp/sealManage/managerSealQuery/markSealDetail') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
