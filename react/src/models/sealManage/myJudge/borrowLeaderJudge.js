...
export default {
  ...
  effects: {
    ...
    * taskInfoSearch({}, {select,call, put}){
      const {passData} = yield select(state=>state.borrowLeaderJudge);
      let recData={
    ...
    * handleModalOk({},{ select,put }){
      const {returnReason} = yield select(state=>state.borrowLeaderJudge);
    ...
    * return({},{call,select,put}){
      const {dataInfo,passData,returnReason} = yield select(state=>state.borrowLeaderJudge);
      let recData={
        ...
        form_if_sign:"",// | char(1) | 是 | 是否需要相关部门会签（0：否，1：是） |
        form_dept_id:dataInfo.form_dept_id,// | char(32) | 是 | 申请或借用部门 |
        form_seal_demand:"",// | varchar(500) | 是 | 刻章规格及要求 |
        form_borrow_date:dataInfo.form_borrow_date,// | timestamp | 是 | 借用时间 |
        form_return_date:dataInfo.form_return_date,// | timestamp | 是 | 归还时间 |
        ...
        if_leader_seal :"1",//| char(1) | 是 | 是否是领导名章审核，0：否，1：是 |
      };
    ...
    * approval({},{call,select,put}){
      const {dataInfo,passData} = yield select(state=>state.borrowLeaderJudge);
      let recData={
        form_title:passData.form_title,
        form_uuid: passData.form_uuid, //           | char(32)    | 是       | 申请单id                                           |
        submit_id :passData.submit_id,//| char(32) | 是 | 提交批次id |
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/myJudge/borrowLeaderJudge') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
