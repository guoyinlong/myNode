...
export default {
  namespace: 'businessLicenseJudge',
  state: {
    dataInfo:[], //详情数据
  ...
  effects: {
    ...
    * fileSearch({}, {select,call, put}){
      const {passData} = yield select(state=>state.businessLicenseJudge);
      let recData={
    ...
    * handleModalOk({},{ select,put }){
      const {returnReason} = yield select(state=>state.businessLicenseJudge);
    ...
    * return({},{call,select,put}){
      const {passData,returnReason} = yield select(state=>state.businessLicenseJudge);
      let recData={
        arg_submit_id :passData.submit_id,//| varchar(32) | 是 | 提交批次id
      ...
      const response = yield call(sealApplyService.businessLicenseRefuse, recData);
      /* console.log(response);*/
      if(response.RetCode === '1'){
        message.info('审核已退回');
    ...
    * approval({},{call,select,put}){
      ...
      const response = yield call(sealApplyService.businessLicenseApproval, recData);
      /* console.log(response);*/
      if(response.RetCode === '1'){
        message.info('审核已通过');
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/myJudge/businessLicenseJudge') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
