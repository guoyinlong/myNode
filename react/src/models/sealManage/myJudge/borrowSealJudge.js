...
export default {
  ...
  effects: {
    ...
    * taskInfoSearch({}, {select,call, put}){
      const {passData} = yield select(state=>state.borrowSealJudge);
      let recData={
    ...
    * handleModalOk({},{ select,put }){
      const {returnReason} = yield select(state=>state.borrowSealJudge);
    ...
    * return({},{call,select,put}){
      const {dataInfo,passData,returnReason} = yield select(state=>state.borrowSealJudge);
      let recData={
    ...
    * approval({},{call,select,put}){
      const {dataInfo,passData} = yield select(state=>state.borrowSealJudge);
      let recData={
        form_title:passData.form_title,
        form_uuid: passData.form_uuid, //           | char(32)    | 是       | 申请单id                                           |
        submit_id :passData.submit_id,//| char(32) | 是 | 提交批次id |
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/myJudge/borrowSealJudge') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
