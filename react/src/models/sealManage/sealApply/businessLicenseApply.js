...
export default {
  namespace: 'businessLicenseApply',
  state: {
  ...
  effects: {
    ...
    * handleDeptOk({},{select,put}){
      ...
      const { dept } = yield select(state => state.businessLicenseApply);
      /*console.log(dept);*/
      yield put({
        type:'save',
        payload:{
    ...
    * saveUploadFile({value},{call,select,put}){
      /* console.log(value);*/
      const {tableUploadFile} = yield select(state => state.businessLicenseApply);
    ...
    * sealSave({},{select,call,put}){
      const { dept,use,sealID, userDept,sealType,fileNumber, fileDay,targetDept, tableUploadFile } = yield select(state=>state.businessLicenseApply);
      let recData={
    ...
    * sendMessages({},{put,call,select}){
      const { sealID } = yield select(state=>state.businessLicenseApply);
      let recData={
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/sealIndexApply/businessLicenseApply') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
