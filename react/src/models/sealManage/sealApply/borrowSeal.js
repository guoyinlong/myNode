...
export default {
  ...
  effects: {
    ...
    * sealSave({},{select,call,put}){
      const { use, startTime, endTime, sealID, sealType, formCheckState} = yield select(state=>state.borrowSeal);
    ...
    * sendMessages({},{put,call,select}){
      const { sealID } = yield select(state=>state.borrowSeal);
      let recData={
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/sealIndexApply/borrowSeal') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
