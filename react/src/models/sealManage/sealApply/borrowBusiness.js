  ...
  effects: {
    ...
    * sealListSearch({},{call,put}){
        let recData={
          ...
          arg_seal_mark:'5',// | char(1) | 是 | 印章唯一标识符（印章：0，领导名章：1，营业执照：2，领导身份证复印件：3，刻章：4，外借营业执照：5）
        }
    ...
    * sealSave({},{select,call,put}){
        const { use, startTime, endTime, sealID, sealType,formCheckState} = yield select(state=>state.borrowBusiness);
    ...
    * sendMessages({},{put,call,select}){
        const { sealID } = yield select(state=>state.borrowBusiness);
        let recData={
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/sealIndexApply/borrowBusiness') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
