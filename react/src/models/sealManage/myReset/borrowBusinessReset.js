...
export default {
  ...
  effects: {
    ...
    * taskInfoSearch({}, {select,call, put}){
      const {passData} = yield select(state=>state.borrowBusinessReset);
      let recData={
          ...
          for(let i = 0; i <res.length;i++){
            ...
            yield put({
              ...
              payload:{
                ...
                startTime:res[i].form_borrow_date.slice(0, -2),  //开始时间
                endTime:res[i].form_return_date.slice(0, -2),   //结束时间
    ...
    * sealListSearch({},{call,put}){
      let recData={
        ...
        arg_seal_mark:'5',// | char(1) | 是 | 印章唯一标识符（印章：0，领导名章：1，营业执照：2，领导身份证复印件：3，刻章：4）
      }
    ...
    * sealSave({},{select,call,put}){
      const { searchData,use, startTime, endTime, sealID, sealNameId} = yield select(state=>state.borrowBusinessReset);
      ...
      if(end < str){
        ...
        let recData = {
          form_title: '营业执照外借申请填报',//  varchar(32)  是  申请单标题
    ...
    * sendMessages({},{put,call,select}){
      const { sealID } = yield select(state=>state.borrowBusinessReset);
      let recData={
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/sealPersonalQuery/borrowBusinessReset') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
