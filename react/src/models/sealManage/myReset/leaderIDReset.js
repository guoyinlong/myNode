...
export default {
  ...
  effects: {
    ...
    * taskInfoSearch({}, {select,call, put}){
      const {passData,dept} = yield select(state=>state.leaderIDReset);
          ...
          for(let i = 0; i <res.length;i++){
            ...
            yield put({
              ...
              payload:{
                ...
                userDept:res[i].form_actual_usename, //实际使用人或部门
                fileNumber:res[i].form_copy_quantity,     //复印件份数
                fileDay:res[i].form_use_day,   //复印件的有效期
                deptInputs:res[i].form_dept_id,   //提交或申请部门
                deptName:res[i].form_dept_name,
                Dept:res[i].form_dept_id,
    ...
    * fileSearch({}, {select,call, put}){
      const {passData,tableUploadFile} = yield select(state=>state.leaderIDReset);
      let recData={
        arg_form_uuid :passData.form_uuid,//| VARCHAR(32)| 是 | 申请单id
    ...
    * localDeleteUpload({record},{call, select, put}){
      const {tableUploadFile} = yield select(state => state.leaderIDReset);
    ...
    * handleDeptOk({},{select,put}){
      ...
      const { dept } = yield select(state => state.leaderIDReset);
      /*console.log(dept);*/
      yield put({
        type:'save',
        payload:{
    ...
    * sealSave({},{select,call,put}){
      const { dept,use,sealID, userDept,sealNameId,fileNumber, fileDay,targetDept, tableUploadFile } = yield select(state=>state.leaderIDReset);
      let recData={
    ...
    * submit({},{select,call,put}){
      const { searchData,dept,use,sealID, userDept,sealNameId,fileNumber, fileDay,targetDept,tableUploadFile } = yield select(state=>state.leaderIDReset);
    ...
    * sendMessages({},{put,call,select}){
      const { sealID } = yield select(state=>state.leaderIDReset);
      let recData={
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/sealPersonalQuery/leaderIDReset') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
