...
export default {
  ...
  effects: {
    ...
    * taskInfoSearch({}, {select,call, put}){
      const {passData} = yield select(state=>state.borrowLeaderReset);
      let recData={
    ...
    * sealSave({},{select,call,put}){
      const { searchData,use, startTime, endTime, sealID, sealNameId} = yield select(state=>state.borrowLeaderReset);
    ...
    * sendMessages({},{put,call,select}){
      const { sealID } = yield select(state=>state.borrowLeaderReset);
      let recData={
  ...
  subscriptions: {
    setup({dispatch, history}) {
      return history.listen(({pathname, query}) => {
        if (pathname === '/adminApp/sealManage/sealPersonalQuery/borrowLeaderReset') { //此处监听的是连接的地址
          dispatch({
            type: 'init',
            query
          });
