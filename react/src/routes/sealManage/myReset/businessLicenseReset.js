...
import DeptRadioGroup from './businessDeptModal.js';
import { Select, Input, Modal, Radio, Button, Table, Popconfirm} from "antd";
import FileUpload from './businessFileUpload.js';        //上传功能组件
...
class BusinessLicenseReset extends React.Component {
  state = {
  ...
  returnModel =(value,value2)=>{
    ...
    if(value2!==undefined){
      this.props.dispatch({
        type:'businessLicenseReset/'+value,
        record : value2,
      })
    }else{
      console.log('aaa');
  ...
  deleteUpload= (e,record)=>{
    /*console.log(record);*/
      this.props.dispatch({
        type:'businessLicenseReset/localDeleteUpload',
        record:record
      })
  ...
  getSealTypeId = (value)=>{
    //console.log(value);
    this.props.dispatch({
      type:'businessLicenseReset/getSealTypeId',
      record : value,
    })
  };
...
function mapStateToProps (state) {
  ...
  return {
    loading: state.loading.models.businessLicenseReset,
    ...state.businessLicenseReset
  };
}
export default connect(mapStateToProps)(BusinessLicenseReset);
