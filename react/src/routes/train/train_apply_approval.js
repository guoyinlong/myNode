...
import { Form, Row, Col, Input, Button, DatePicker, Select, Modal, Table, message, Card, Radio } from 'antd';
import { routerRedux } from "dva/router";
import { connect } from "dva";
import styles from "../overtime/style.less";
import UpFileApply from "./upFileApplyCourse";
import ExcelPersonGrade from "./ExcelPersonGrade";
...
class CreateTrainApplyeApproval extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: false,
  ...
  selectNext = () => {
    ...
    if (approvalInfoRecord.step === "接口人提交执行培训" || approvalInfoRecord.step === '申请人组织培训') {
      //判断是否是申请人组织培训
      let trainOrgan = this.props.form.getFieldValue("trainOrgan");
      ...
      let is_purchase = this.state.is_purchase;
      ...
      let train_teacher = this.props.form.getFieldValue("train_teacher");

      if (trainOrgan === '' || trainOrgan === undefined || trainOrgan === null) {
        message.error("请输入培训机构");
        return;
      } else if (trainUseFee === '' || trainUseFee === undefined || trainUseFee === null) {
        message.error("请输入培训费用");
        return;
      } else if (trainNumberPerson === '' || trainNumberPerson === undefined || trainNumberPerson === null) {
        message.error("请输入参训人数");
        return;
      } else if (is_purchase === '' || is_purchase === undefined || is_purchase === null) {
        message.error("请选择是否进行采购");
        return;
        ...
        return;
      } else if (train_teacher === '' || train_teacher === undefined || train_teacher === null) {
        message.error("请填写培训讲师");
        return;
  ...
  handleOk = () => {
    ...
    let arg_is_out_of_plan = approvalInfoRecord.is_out_of_plan;
    ...
    if (approvalInfoRecord.step === "接口人提交执行培训" || approvalInfoRecord.step === '申请人组织培训') {
      ...
      //如果是计划外培训，则校验信息后补充 train_class_personal_info表 state置为2
      if (approvalInfoRecord.is_out_of_plan === '1') {
        ...
        transferPersonClassGradeData.map((item) => {
          let personClassGradeData = {
            ...
            //地区
            arg_train_area: item.train_area,
            //用户名
            arg_login_name: item.login_name,
            //考试成绩
            arg_class_grade: item.class_grade,
            //是否合格
            arg_if_pass: item.if_pass,
            //是否是计划外培训
            arg_is_out_of_plan: approvalInfoRecord.is_out_of_plan,
            //arg_proc_inst_id
            arg_proc_inst_id: approvalInfoRecord.proc_inst_id,
            //培训类型
            arg_train_class_type: approvalInfoRecord.train_class_type,
      ...
      //计划内培训，直接导入人员信息到train_class_personal_info state置为2
        ...
        transferPersonClassGradeData.map((item) => {
          let personClassGradeData = {
            //课程成绩ID
            arg_train_class_personal_info_id: train_class_personal_info_id,
            //成绩信息：
            //参训人员
            arg_user_name: item.user_name,
            //地区
      ...
      //校验计划内、计划外导入的人员数量是否大于申请的人员数量
      if (transferPersonClassGradeList.length > trainNumberPerson) {
        message.error("您导入的参训人员大于输入的参训人员数量,请返回修改后提交!");
        this.setState({ isSubmitClickable: true });
        return;
      }
      //开始提交信息
      return new Promise((resolve_out) => {
        //提交课程人员信息
        dispatch({
          type: 'createApprovalModel/trainClassPersonApprovalSubmit',
        ...
        if (resolve_out === 'success') {
          ...
          if (pf_url && file_relative_path && file_name && pf_url != '' && file_relative_path != '' && file_name != '') {
            ...
            dispatch({
              type: 'createApprovalModel/trainClassExecFileSubmit',
              exec_data,
              arg_proc_inst_id,
          ...
          if (approval_if === '不同意' && approval_advice === '') {
            ...
            return new Promise((resolve) => {
              dispatch({
                type: 'createApprovalModel/trainClassApprovalSubmit',
                approval_if,
                approval_advice,
                nextstepPerson,
                nextstep,
  ...
  render() {
    ...
    if (approvalNowList.length > 0 && approvalNowList[0].task_name.endsWith("归档")) {
      ...
      if (approvalNowList.length > 0 && approvalNowList[0].task_name.endsWith("结束")) {
        nowdataList = '';
      } else { 
    ...
    console.log(approvalInfoRecord)
    console.log('PPPPPPP')
    ...
    return (
      <div>
        <Row span={2} style={{ textAlign: 'center' }}><h1>外训培训申请审批表</h1></Row>
        <br /><br />
        <Form>
          <Card title="申请信息" className={styles.r}>
            ...
            <Row style={{ textAlign: 'left' }}>
              <FormItem label={'培训开始时间'} {...formItemLayout}>
                {getFieldDecorator('train_start_time', {
                  //initialValue:moment(approvalInfoRecord.train_start_time,'YYYY-MM-DD'), 
            ...
            <Row style={{ textAlign: 'left' }}>
              <FormItem label={'培训结束时间'} {...formItemLayout}>
                {getFieldDecorator('train_end_time', {
                  //initialValue:moment(approvalInfoRecord.train_end_time,'YYYY-MM-DD'),
            ...
            {/*申请人执行*/} 
            <Card title="组织培训情况填报"
              style={{ display: (approvalInfoRecord.step === "接口人提交执行培训" || approvalInfoRecord.step === "申请人组织培训" || approvalInfoRecord.step === "部门负责人审核" || approvalInfoRecord.step === "部门负责人审核-执行完" || approvalInfoRecord.step === "人力资源专员归档") ? " " : "none" }}
            >
              {/*培训机构*/}
              <Row style={{ textAlign: 'center' }}>
                <FormItem label="培训机构" hasFeedback {...formItemLayout}>
                  {getFieldDecorator('trainOrgan', {
                    initialValue: approvalInfoRecord.trainOrgan,
                    rules: [
                      {
                        required: true,
                        message: '请填写培训机构'
                    ...
                    <Input style={{ color: '#000' }} initialValue={approvalInfoRecord.trainOrgan} disabled={approvalInfoRecord.step === "接口人提交执行培训" || approvalInfoRecord.step === "申请人组织培训" ? false : true} />
                  )}
                </FormItem>
              </Row>
              <Row style={{ textAlign: 'center' }}>
              ...
              <Row style={{ textAlign: 'left' }}>
                <FormItem label="是否采购" hasFeedback {...formItemLayout}>
                  {getFieldDecorator('is_purchase', {
                    initialValue: approvalInfoRecord.is_purchase,
                    rules: [
                      {
                        required: true,
                        message: '请选择是否进行采购'
                    ...
                    <Select style={{ color: '#000' }} onChange={this.isPurchase} placeholder="" disabled={approvalInfoRecord.step === "接口人提交执行培训" || approvalInfoRecord.step === "申请人组织培训" ? false : true}>
              ...
              <Row style={{ textAlign: 'center' }}>
                <FormItem label="请填写培训讲师（姓名）" hasFeedback {...formItemLayout}>
                  {getFieldDecorator('train_teacher', {
                    initialValue: approvalInfoRecord.train_teacher,
                    rules: [
                      {
                        required: true,
                        message: '请填写培训讲师（姓名）'
...
CreateTrainApplyeApproval = Form.create()(CreateTrainApplyeApproval);
export default connect(mapStateToProps)(CreateTrainApplyeApproval)
