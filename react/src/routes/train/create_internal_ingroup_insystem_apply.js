...
class CreateInternalIngroupInsystemTrainApplye extends React.Component {
  constructor(props) {
    super(props);
        ...
        if (err) {
          ...
          if (trainStartTime && trainEndTime) {
            ...
            if (formData.trainTeacherInput === '') {
              message.error('请填写集团或系统内培训师姓名，以空格分隔！');
              return;
            }
        ...
        if (trainStartTime && trainEndTime) {
          ...
          if (this.diffDate(currentDate, trainStartTime)) {
            ...
            if (this.state.planValue === 1) {
              ...
              //培训申请类型
              basicOutPlanApplyData["arg_train_apply_type"] = '1';
            } else {
              //封装培训课程信息传给后台
              ...
              //培训申请类型
              basicApplyData["arg_train_apply_type"] = '1';
            }
  ...
  applyClassType = (value) => {
    ...
    let queryParam = {
      ...
      arg_train_class_type: "参加集团或分子公司培训",
    };
    return new Promise((resolve) => {
      dispatch({
        //全院级必修课保存
  ...
  render() {
    ...
    return (
      <div>
        <Row span={2} style={{ textAlign: 'center' }}><h1>内训-参加集团或分子公司培训申请表</h1></Row>
        <br /><br />
        <Form>
          <Card title="申请信息" className={styles.r}>
            ...
            {/* 培训课程 */}
            {
              this.state.planValue === 0
                ...
                <Row style={{ textAlign: 'center' }}>
                  <Col >
                    <FormItem label="请选择计划外培训类型" hasFeedback {...formItemLayout}>
                        ...
                        rules: [
                          {
                            ...
                            message: '请输入计划外培训课程的计划培训时长(小时，6小时为1天，最多2位小数)'
                          },
                        ],
                      })(
                        <Input placeholder="请输入计划外培训课程的计划培训时长(小时，6小时为1天，最多2位小数)" />
                      )}
                    </FormItem>
...
CreateInternalIngroupInsystemTrainApplye = Form.create()(CreateInternalIngroupInsystemTrainApplye);
export default connect(mapStateToProps)(CreateInternalIngroupInsystemTrainApplye)
