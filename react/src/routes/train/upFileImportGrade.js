...
class UpFileImportGrade extends React.Component {
  constructor(props) {
    super(props);
  }
  componentWillMount() {
  ...
  handleChange = (info) => {
    ...
    if (info.file.status === 'removed') {
      ...
      if (this.state.newfile.uid === 1) {
        ...
        dispatch({
          type: 'importGradeModel/deleteFile',
          RelativePath
        });
      } else {
      ...
      if (info.file.status === 'done') {
        ...
        dispatch({
          type: 'importGradeModel/changeNewFile',
          oldfile,
          newfile,
        });
...
export default connect()(UpFileImportGrade);
