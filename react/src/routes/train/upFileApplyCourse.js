...
class UpFileApplyCourse extends React.Component {
  constructor(props) {
    super(props);
  }
  componentWillMount(){
  ...
  handleChange = (info) => {
    ...
    if(info.file.status === 'removed') {
      ...
      if(this.state.newfile.uid===1){
        ...
        dispatch({
          type:'createApprovalModel/deleteFile',
          RelativePath
        });
      }else{
      ...
      if (info.file.status === 'done') {
        ...
        dispatch({
          type:'createApprovalModel/changeNewFile',
          oldfile,
          newfile,
        });
...
export default connect()(UpFileApplyCourse);
