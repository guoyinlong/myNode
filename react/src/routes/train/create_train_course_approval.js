...
import ExcelPersonGrade from "./ExcelPersonGrade3";
...
class CreateTrainCourseApproval extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: false,
  ...
  handleOk = () => {
    ...
    if (approvalInfoRecord.step === '接口人提交执行培训' || approvalInfoRecord.step === "申请人组织培训") {
      ...
      new Promise((resolve1) => {
        dispatch({
          type: 'createApprovalModel/trainCourseClassPersonApprovalSubmit',
        ...
        if (resolve1 === 'false') {
          ...
          if (approval_if === '不同意' && approval_advice === '') {
            ...
            return new Promise((resolve) => {
              dispatch({
                type: 'createApprovalModel/trainCourseClassApprovalSubmit',
                approval_if,
                approval_advice,
                nextstepPerson,
                nextstep,
  ...
  render() {
    ...
    if (approvalNowList.length > 0 && approvalNowList[0].task_name.endsWith("归档")) {
      ...
      if (approvalNowList.length > 0 && approvalNowList[0].task_name.endsWith("结束")) {
        ...
        if (approvalNowList.length > 0 && (approvalNowList[0].task_name.endsWith("内训师确认") || approvalNowList[0].task_name.endsWith("人力接口人确认"))) {
      ...
      CourseClassInfo = courseClassData.map(item =>
        <Col span={8}>
          <Card style={{ textAlign: 'left', width: '100%' }} type="inner">
            ...
            <p>课程类型：{item.train_class_apply_type === '1' ? '内训-参加集团及系统内培训' : (item.arg_train_class_apply_type === '2' ? '内训-自有内训师' : '内训-外请讲师')}</p>
    ...
    return (
      <div>
        <Row span={2} style={{ textAlign: 'center' }}><h1>培训班审批表</h1></Row>
        <br /><br />
        <Form>
          <Card title="培训班申请信息" className={styles.r}>
            ...
            {/* <Row style={{ textAlign: 'center' }}>
              <FormItem label="计划培训人数" hasFeedback {...formItemLayout}>
                {getFieldDecorator('train_class_person_num', {
                  initialValue:approvalInfoRecord.train_class_person_num,
                })(
                  <TextArea
                    style={{color:'#000', minHeight:32 }}
                    placeholder="计划培训人数"
            ...
            {/*申请人执行*/}
            <Card title="组织培训情况填报"
              style={{ display: (approvalInfoRecord.step === "接口人提交执行培训" || approvalInfoRecord.step === "申请人组织培训" || approvalInfoRecord.step === "部门负责人审核" || approvalInfoRecord.step === "部门负责人审核-执行完" || approvalInfoRecord.step === "人力接口人归档") ? " " : "none" }}
            >
              <Row style={{ textAlign: 'center' }}>
...
CreateTrainCourseApproval = Form.create()(CreateTrainCourseApproval);
export default connect(mapStateToProps)(CreateTrainCourseApproval)
