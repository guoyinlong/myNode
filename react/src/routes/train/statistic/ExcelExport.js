...
var exportExl = function (SourceData,Name,condition) {
  ...
  function  base64(s){ return window.btoa(unescape(encodeURIComponent(s))) }
    ...
    for (let i = 0; i < SourceData.length; i++) {
      ...
      // let personData = {
      //   indexID: i+1,
      //   class_type: SourceData[i].class_type,
      //   class_name: SourceData[i].class_name,
      //   train_year: SourceData[i].train_year,
      //   train_time: SourceData[i].train_time,
      //   train_fee: SourceData[i].train_fee,
      //   class_grade: SourceData[i].class_grade,
      //   dept_name:SourceData[i].dept_name,
    ...
    for (let j = 0; j < personDataListExportTemp.length; j++) {
      ...
      for (let item in personDataListExportTemp[j]) {
        ...
        content += `<td style="vnd.ms-excel.numberformat:@">${personDataListExportTemp[j][item]}</td>`;
      }
      content += '</tr>';
    }
    //Worksheet名
