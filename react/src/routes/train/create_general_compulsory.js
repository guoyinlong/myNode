...
class Create_general_compulsory extends Component{
  constructor (props) {
    super(props);
  ...
  saveAction = () => {
    ...
    const{dispatch} = this.props;
    let planList = this.props.importCenterClassCompulsoryDataList;
    /*非空校验*/
    ...
    return new Promise((resolve) => {
      dispatch({
        //全院级必修课保存
        type:'train_create_model/centerClassCompulsorySubmit',
        transferPlanList,
        trainPlanData,
        plan_id,
        resolve
  ...
  render() {
    const classDataList = this.props.importCenterClassCompulsoryDataList;
    ...
    return(
      <div>
        ...
        <div style={{ float: 'left'}}>
          <a href="/filemanage/download/needlogin/hr/compulsory_class_plan.xls" ><Button >{'全院级（必修）培训计划模板下载'}</Button></a>
          &nbsp;&nbsp;&nbsp;&nbsp;
        ...
        <Row span={2} style={{textAlign: 'center'}}><h2>{new Date().getFullYear()}年全院级（必修课）培训计划</h2></Row>
        <br/>

        <Card title="全院级（必修课）培训计划课程信息">
          <br/>
          <br/>
          <Table
...
Create_general_compulsory = Form.create()(Create_general_compulsory);
export default connect(mapStateToProps)(Create_general_compulsory);
