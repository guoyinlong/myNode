...
import {Button,DatePicker,TreeSelect, Icon, Radio, Input, Select ,Modal,Table,Popconfirm} from 'antd'
import { ContentUtils } from 'braft-utils'
import { ImageUtils } from 'braft-finder'
// import Upload from './fileUpload';        //上传功能组件
...
class manuscriptRevision extends React.PureComponent {
	state = {
	...
	changeDate = (date,dateString) => {
			this.props.dispatch({
				type: 'manuscriptRevision/changeDate',
...
setEditorContentAsync = () => {
  const {outputHTML } = this.props;
  this.isLivinig && this.setState({
    editorState: BraftEditor.createEditorState(outputHTML),
    tuloadoing:false,
  })
}
componentDidMount () {
  ...
  setTimeout(this.setEditorContentAsync, 1000)
	...
	//----------------------页面渲染----------------------//
	render() {
    const { checkObjectAndContentList, checkContentList, qudaoDataList,author,authorList,examineImgId,outputHTML,querySecretFileList} = this.props;
    const { editorState,tuloadoing } = this.state
        //单位
        ...
        return(
            <div className={styles.pageContainer}>
						<h2 style = {{textAlign:'center',marginBottom:30}}>稿件修改</h2>
              <div className={styles.lineOut}>
                      <span className={styles.lineKey}>
                      <b className={styles.lineStar}>*</b> 稿件名称
                      </span>
                      <span className={styles.lineColon}>：</span>
                      <Input style={{width:'570px'}} placeholder = "请输入" value={this.props.theme}  autofocus="autofocus" onChange={(e)=>this.returnModel('theme',e)}/>
              </div>
            <div className={styles.lineOut}>
                    <span className={styles.lineKey}>
            ...
            <div className={styles.lineOut}>
                  ...
                  {/* <div style={{width:800,height:500,overflow:"hidden",display:"inline-block"}}>
                      ...
                      style={{width:"100%",height:"100%",backgroundColor:"rgb(245 244 244)"}}
                        // value={outputHTML1!=""?outputHTML1:editorState} 
                        value={editorState}
                ...
                <FileUpload dispatch={this.props.dispatch} passFuc = {this.saveData} pageName='manuscriptRevision'/>
                <Table
                      columns={ columns }
...
function mapStateToProps (state) {
  return {
    loading: state.loading.models.manuscriptRevision, 
    ...state.manuscriptRevision
  };
}

export default connect(mapStateToProps)(manuscriptRevision);
