...
import {Input, DatePicker, TreeSelect, Radio, Button, Upload, Table, Popconfirm, message, Modal, Col} from 'antd';
import styles from "../../index.less";
import Cookie from "js-cookie";
const dateFormat = 'YYYY-MM-DD';
class trainRecordUpload extends Component {
  constructor(props){
    super(props);
    this.state= {
      uploadFile:{
  ...
  render() {
    const {cultosiName, staffName,cultiTime, checkObject,checkObjectAndContentList, typeValue, pictureList, checkContentList} =this.props;
    //  考试题目回答情况
    ...
    return(
      <div  className={styles.outerField}>
        <div className={styles.out}>
          <h2 style={{margin:'0 auto',width:'800px',textAlign:'center',fontSize:'20px'}}>培训备案上传</h2>
          ...
          <div style={{marginTop:'20px'}}>
            ...
            <TreeSelect
              ...
              value={staffName}
              labelInValue={true}
              onChange={(e)=>this.handleMole('staffVlaue',e)}
            >
            </TreeSelect>
          </div>
          <div style={{marginTop:'20px'}}>
              ...
              typeValue <=0 ?
                <div>
                  ...
                  <Upload  {...this.state.uploadFile}
                           onChange = {(e)=>this.handleMole("trainingMaterialChange",e)}
                ...
                <div>
                  ...
                  <span style={{display:'inline-block', width:'10px',textAlign: 'left',marginLeft:'5px',marginRight:'10px'}}>:</span>
                  <Upload   {...this.state.uploadFile}
                            onChange = {(e)=>this.handleMole("questionnaireChange",e)}
                            beforeUpload={this.beforeUpload}
                  >
                    <Button type="primary">上传</Button>
                    <i style={{color:"red",marginLeft:'15px',fontStyle:"normal"}}>推荐上传截图、图片等</i>
...
function mapStateToProps (state) {
  return {
    loading: state.loading.models.trainRecordUpload,
    ...state.trainRecordUpload
  };
}

export default connect(mapStateToProps)(trainRecordUpload);