...
class newPublicity extends React.PureComponent {
  state={ }
    returnModel =(value,value2)=>{
        if(value2!==undefined){
            this.props.dispatch({
                type:'newPublicity/'+value,
                record : value2,
            })
        }else{
            this.props.dispatch({
	...
	//----------------------页面渲染----------------------//
	render() {
        const {  duizhangList,duiyuanList} = this.props;
        //队长
	...
	return(
            <div className={styles.pageContainer}>
						<h2 style = {{textAlign:'center',marginBottom:30}}>宣传组织填报</h2>
...
function mapStateToProps (state) {
  return {
    loading: state.loading.models.newPublicity, 
    ...state.newPublicity
  };
}
export default connect(mapStateToProps)(newPublicity);
