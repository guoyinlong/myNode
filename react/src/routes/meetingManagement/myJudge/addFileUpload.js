...
class FileUpload extends React.Component {
  ...
  updateFilePath = (value) => {
    this.props.dispatch({
      type: 'addFile/saveUploadFile',
      value: value
    })
  };
  noUpdate = () => {
    this.props.dispatch({
      type: 'addFile/noUpdate',
    })
  }
  ...
  render() {
    return (
      <Upload {...this.state.uploadFile} showUploadList= {false} accept=".doc,.txt,.pdf,.docx">
        <Button type="primary" onClick={this.noUpdate}>上传</Button>
        <i style={{color:"red",marginLeft:'15px'}}>推荐推荐pdf、不能上传压缩包</i>
      </Upload>
    )
  }
}
