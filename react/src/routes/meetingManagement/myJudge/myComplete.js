...
class MyComplete extends React.Component {
  ...
  downPage = () => {
    this.props.dispatch({
      type: 'myComplete/downPaper', //走终止议题的服务
    })
  };
                  ...
                  this.props.reasonImportant==='1'?
                    <div style={{marginTop:'15px'}} >
                     <span style={{display:'inline-block', width:'160px',textAlign: 'right'}}>
                      ...
                      <span style={{display:'inline-block', width:'10px',textAlign: 'left'}}>:</span>
                      <span>{this.props.tableLineDetail.map((item)=><i>{item.topic_important_reason}</i>)}</span>
                    </div>
...
function mapStateToProps(state) {
  ...
  return {
    loading: state.loading.models.myComplete,
    ...state.myComplete
  };
}
export default connect(mapStateToProps)(MyComplete);