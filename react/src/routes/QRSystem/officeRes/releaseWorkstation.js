...
import styles from './releaseWorkstation.less';
import { Button, Modal, Table, message, DatePicker, Row, Col, Icon, Input } from 'antd';
...
class ReleaseWorkstation extends React.Component {
  ...
  //    })
  //  }
   getNewData = (releaseList,searchObject) => {
  ...
  beginTime = (date, dateString) => {
    if (dateString[0] == "" && dateString[1] == "") {
      this.props.dispatch({
        type: "releaseWorkstation/clearApplyDate"
      });
    } else {
      this.props.dispatch({
        type: "releaseWorkstation/beginTime",
        data: dateString
      });
    };
  };
  ...
  endTime = (date, dateString) => {
    if (dateString[0] == "" && dateString[1] == "") {
      this.props.dispatch({
        type: "releaseWorkstation/clearExpireDate"
      });
    } else {
      this.props.dispatch({
        type: "releaseWorkstation/endTime",
        data: dateString
      });
    };
  };
  ...
  // 释放按钮
  goToSubmit = () => {
    this.props.dispatch({ type: "releaseWorkstation/flowSubmit" })
  }
  //勾选释放人员申请数据
  rowSelect = (selectedRowKeys, selectedRows) => {
    this.props.dispatch({ type: "releaseWorkstation/releaseRowSelect", data: selectedRows })
  }
  ...
  render() {
    ...
    //const releaseList = this.getNewData(this.props.releaseList, this.state.dataIndex, this.state.searchText)
    const rowSelect = { onChange: this.rowSelect }
    const modalHtml = this.getDetailModalHtml();
    const dataSource = this.getNewData(releaseList,this.state.searchContent);
    return (
      <div className={styles.wrapper + ' ' + styles.delayWorkstation}>
        <h2 style={{ textAlign: 'center', marginBottom: 30 }}>释放工位</h2>
        <div className={styles.buttonWrapper}>
          <div>
            <span>申请部门：{Cookie.get("deptname")}</span>
          </div>
          <div >
            <Button type="primary" className={styles.buttonAdd} onClick={this.goToSubmit} >释放</Button>
...
function mapStateToProps(state) {
  return {
    loading: state.loading.models.releaseWorkstation,
    ...state.releaseWorkstation
  }
}
export default connect(mapStateToProps)(ReleaseWorkstation);
