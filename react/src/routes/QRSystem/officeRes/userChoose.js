...
export default class EmpList extends React.Component {
  ...
  onChangeChildren=(value, label, extra)=>{
    if(this.props.occupyState === '闲置' || this.props.occupyState === '不可用'){
      message.info("请将状态修改为长期占用或机动备用");
      return;
    }
      ...
      return (
        <div style={{...style,display:'inline-block'}} >
          <TreeSelect
            ...
            // value={this.state.value?this.state.value:this.props.initialData}
            // value = {window.locUserValue?window.locUserValue:this.props.initialData}
