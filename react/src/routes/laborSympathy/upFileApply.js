...
class UpFileApply extends React.Component {
  constructor(props) {
    super(props);
  }
  componentWillMount() {
  ...
  handleChange = (info) => {
    ...
    if (info.file.status === 'removed') {
      ...
      if (this.state.newfile.uid === 1) {
        ...
        dispatch({
          type: 'labor_sympathy_apply_model/deleteFile',
          RelativePath
        });
      } else {
      ...
      if (info.file.status === 'done') {
        ...
        dispatch({
          type: 'labor_sympathy_apply_model/changeNewFile',
          oldfile,
          newfile,
        });
...
export default connect()(UpFileApply);
