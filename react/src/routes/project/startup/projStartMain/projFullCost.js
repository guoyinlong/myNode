...
class ProjFullCost extends React.PureComponent {
     ...
     * 功能：点击编辑图标触发
     */
    clickFullCostEditIcon = () => {
        this.props.dispatch({
            type:'projStartMainPage/clickFullCostEditIcon',
        });
    };
    ...
    hideChangeModal=(flag)=>{
        if(flag === 'confirm'){
            if(this.props.fullCostEditFlag === '3'){
                this.props.dispatch({
        ...
        this.props.dispatch({
            type:'projStartMainPage/closeFullCostModal',
        });
    };
    ...
    tabChangeClick=(value) => {
        ...
        if(this.props.from_page==="start"){
            if (value !== this.props.fullCostPmsTab.tabConvertName) {
                this.props.dispatch({
                    type:'projStartMainPage/fullCostPmsTabClick',
                    value
                });
            }
        }
        
         if(this.props.from_page==="History"){
            if (value !== this.props.fullCostPmsTab.tabConvertName) {
                 this.props.dispatch({
                 type:'projHistoryDetail/fullCostPmsTabClick',
                    value
                });
           }
            }
    ...
    render(){
        ...
        //编辑图标显示的信息
        let toolTip = '';
        if(this.props.fullCostEditFlag === '1' || this.props.fullCostEditFlag === '3'){
            toolTip = <Tooltip title="编辑">
                <Icon type='bianji'
                      style={{color:'blue',cursor:'pointer',fontSize:20}}
                      onClick={this.clickFullCostEditIcon}
                />
            </Tooltip>;
        }else if(this.props.fullCostEditFlag === '2'){
            toolTip = <Tooltip title={this.props.fullCostEditVal}>
                <Icon type='bianji'
