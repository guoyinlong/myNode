 ...
 * 文件说明：实现项目启动里面对已立项的全成本编辑功能（TMO有权限）
 */
import {Modal,Table,Icon,Button,Input,Select,message} from 'antd';
import AssignDept  from '../../../../components/commonApp/assignDept.js';
import ChoseVerifier from './fullCostModule/choseVerifier';
import styles from '../projAdd/projStartFullCost.less';
 ...
 * 功能：实现项目启动全程本编辑
 */
class ProjApproveEditFullCost extends React.PureComponent {
  ...
  hideDeptModal=(flag)=>{
    if(flag === 'confirm'){
      ...
      //新增配合部门信息
      this.props.dispatch({
        type:'projStartMainPage/addCoorpDept',
  ...
  hidePurchaseModal=(flag)=>{
    if(flag === 'confirm'){
      ...
      if(isInArray(this.props.yearListRowSpan[this.state.year].purchaseCostList,this.state.purchaseValue)){
        ...
        this.props.dispatch({
          type:'projStartMainPage/addCostType',
  ...
  hideYearModal=(flag)=>{
    if(flag === 'confirm'){
      ...
      if(isInArray(this.props.yearList,this.state.yearValue)){
        ...
        this.props.dispatch({
          type:'projStartMainPage/addYear',
  ...
  hideVerifyModal=(flag)=>{
    if(flag === 'confirm'){
      ...
      if(verifierValue === '' || verifierValue === undefined){
        ...
        this.props.dispatch({
          type:'projStartMainPage/submitFullCostBudget',
  ...
  deleteCoorpDept=(index)=>{
    this.props.dispatch({
      type:'projStartMainPage/deleteCoorpDept',
      index:index
    });
  };
  ...
  editCoorpMgrName=(e,index)=>{
    this.props.dispatch({
      type:'projStartMainPage/editCoorpMgrName',
      index:index,
      text:e.target.value
    });
  };
  ...
  editCellData = (e,year,deptName,noPreFeeName) => {
    ...
    this.props.dispatch({
      type:'projStartMainPage/editCellData',
      value:value,
      year:year,
      deptName:deptName,
  ...
  deleteCostType = (row) =>{
    this.props.dispatch({
      type:'projStartMainPage/deleteCostType',
    ...
    confirm({
      ...
      onOk() {
        thisMe.props.dispatch({
          type:'projStartMainPage/deleteYear',
          year:year
        });
      },
      onCancel() {
   ...
   */
  changeFullCostShow = (pageToType) => {
    this.props.dispatch({
      type:'projStartMainPage/changeFullCostShow',
  ...
  goBackView = () => {
    ...
    if(coorDeptData.length === 0 && deptBudgetData.length === 0){
      this.changeFullCostShow('view');
    }else{
      let thisMe = this;
      confirm({
        title:config.CONTENT_CHANGE,
        onOk() {
          thisMe.changeFullCostShow('view');
        },
        onCancel() {
        },
      });
  ...
  submitFullCost = () =>{
    ...
    let deptHasInsOrDel = false;     //判断配合部门中有无新增(insert)或者删除(delete)的数据
    if (arrayProjDept.length > 0 ){
      for (let di = 0; di < arrayProjDept.length; di++) {
        if (arrayProjDept[di].opt_type === 'insert' || arrayProjDept[di].opt_type === 'delete') {
          deptHasInsOrDel = true;
          break;
        }
      }
    }
    //如果只是修改配合部门联系人的名字（update），且预算数据没有发生变化
    if (arrayProjDept.length > 0 && deptHasInsOrDel === false && arrayProjBudget.length === 0) {
      //message.info('正确');
      let object_cos = {};
      object_cos.array_proj_dept = arrayProjDept;
      object_cos.array_proj_budget = [];
      this.props.dispatch({
        type:'projStartMainPage/submitFullCostDept',
        object_cos:object_cos,
        reasonValue:this.state.reasonValue.trim()
      });
    } else if ((arrayProjDept.length > 0 && deptHasInsOrDel === true) || arrayProjBudget.length > 0) {
      //如果配合部门中有 insert 或者 delete,或者预算中修改过的数据，显示审核人对话框
      this.props.dispatch({
        type:'projStartMainPage/submitGetVerifierData',
      });
...
export default ProjApproveEditFullCost;
