 ...
 * 功能：里程碑时的模态框
 */
class MileStoneAddOrEdit extends React.Component {

    state = {
        planWorkload: this.props.mileModalType === 'add' ? 0 : this.props.mileStoneModalData.plan_workload,
        planWorkloadOriginal: this.props.mileModalType === 'add' ? 0 : this.props.mileStoneModalData.plan_workload,
    };
    ...
    planWorkloadCheck = (rule, value, callback) => {
        if (this.props.mileModalType === 'add') {
        ...
        else if (this.props.mileModalType === 'edit') {
            ...
            for (let i = 0; i < this.props.mileStoneList.length; i++) {
                ...
                if (this.props.mileModalType === 'add') {
                    if(this.props.mileStoneList[i].opt_type !== 'delete'){
                        if(this.props.mileStoneList[i].mile_name === value.trim()){
                            callback('新添加的里程碑名称不能与已经存在的里程碑名称相同');
                        }
                    }
                }
                else if (this.props.mileModalType === 'edit') {
                    if(this.props.mileStoneList[i].opt_type !== 'delete' && i !== this.props.mileStoneRecord.key){
                        if(this.props.mileStoneList[i].mile_name === value.trim()){
                            callback('新添加的里程碑名称不能与已经存在的里程碑名称相同');
                        }
                    }
    ...
    render() {
        ...
        const { mileStoneModalData } = this.props;
        return (
            <Form>
                <Row gutter={16}>
                    <Col className="gutter-row">
                        <FormItem label="里程碑名称" {...formItemLayout1} >
                            {getFieldDecorator('mile_name', {
                                ...
                                initialValue: mileStoneModalData.mile_name ? mileStoneModalData.mile_name : ''
                            })
                    ...
                    <Col className="gutter-row" {...FormItemCol.preCol}>
                        <FormItem label="开始时间" {...formItemLayout2}>
                            {getFieldDecorator('plan_begin_time', {
                                ...
                                initialValue: mileStoneModalData.plan_begin_time ? moment(mileStoneModalData.plan_begin_time) : null
                            })
                    ...
                    <Col className="gutter-row" {...FormItemCol.preCol}>
                        <FormItem label="结束时间" {...formItemLayout2} >
                            {getFieldDecorator('plan_end_time', {
                                ...
                                initialValue: mileStoneModalData.plan_end_time ? moment(mileStoneModalData.plan_end_time) : null
                            })
                ...
                <Row gutter={16}>
                    ...
                    <Col className="gutter-row" {...FormItemCol.preCol}>
                        <FormItem label="待分配工作量" {...formItemLayout2} >
                            {
                                this.props.mileModalType === 'add'
                                    ?
                                    <span>{(this.props.remainWorkLoad - this.state.planWorkload).toFixed(1)}</span>
                                    :
                                    <span>
                                        {
                                            (   Number(this.props.remainWorkLoad) +
                                                Number(this.state.planWorkloadOriginal) -
                                                Number(this.state.planWorkload)
                                            ).toFixed(1)
                                        }
                                    </span>
                            }
...
export default Form.create()(MileStoneAddOrEdit);
