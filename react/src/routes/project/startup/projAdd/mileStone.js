...
import MileAddOrEdit from './mileAddOrEdit';
...
class MileStone extends React.Component {
    ...
    showMileModal = (modalType, mileStoneRecord) => {
        this.props.dispatch({
            type: 'projectInfo/showMileModal',
            modalType,
            mileStoneRecord
        });
    };
    ...
    deleteMilestone = (record) => {
        //删除时，可能需要有限制
        this.props.dispatch({
            type: 'projectInfo/deleteMilestone',
        ...
        if (flag === 'confirm') {
            this.refs.mileAddOrEdit.validateFields((err, values) => {
                if (err) {
                    ...
                    dispatch({
                        type: 'projectInfo/hideMileModal',
                        flag,
                        mileParams: {
        ...
        for (let i = 0; i < mileStoneList.length; i++) {
            ...
            switch (mileStoneList[i].opt_type) {
                ...
                case 'update':
                    ...
                    //所以此处传的值不会传空字段
                    obj.flag = 'update';
                    obj.mile_form_id = mileStoneList[i].mile_form_id;
                    //如果里程碑名称改变了，传值
    ...
    render() {
        const {mileStoneList, fore_workload, remainWorkLoad, mileModalType, begin_time, end_time} = this.props;
        let columns = [
            {
                title: '序号',
                dataIndex: '',
