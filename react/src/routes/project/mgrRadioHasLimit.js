 ...
 *  文件说明：实现选择项目经理的功能
 */
import { Row, Col, Tree,Icon,message,Spin,Tooltip} from 'antd';
...
class MgrRadioGroup extends React.Component {
    ...
    onSelect = (selectedKeys, info) => {
        ...
        if (selectedKeys[0] == undefined) {
            ...
            if (selectedKeys[0].length == 7) {
                ...
                mgrInfo = info.selectedNodes[0].props.title.props.children;    //由于使用自定义title,此处修改过
    ...
    componentDidMount() {
        ...
        let oudata = request('/microservice/allproject/project/ProjectInitiationInfoProjectManagerQuery');
        oudata.then((data)=> {
            data.DataRows = JSON.parse(data.DataRows);
            let ouDeptList = {};
                                                ...
                                                >
                                                    {
                                                        cCItem.child.map((cCCItem, cCCIndex)=>{
                                                            ...
                                                            if (cCCItem.is_add === '0') {
                                                                titleData = (
                                                                    <Tooltip title={cCCItem.is_add_info}>
                                                                        <span>
                                                                            {cCCItem.dept_name}
                                                                        </span>
                                                                    </Tooltip>
