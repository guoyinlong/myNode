 ...
 *  文件说明：项目变更中，申请人第一次申请页面中的项目基本信息tab
 */
...
//import styles from './projChangeExamingInfo.less'
...
class ProjInfo extends React.Component {
    ...
    hideMainDeptModel = () => {
        ...
        // 判断主责部门是否变化，并获取即将执行操作的提示
        if(mainDeptName!==this.props.projOldInfo.dept_name){
            let postData = {
                arg_proj_id: this.props.arg_proj_id, // 项目id
                arg_proj_uid: this.props.projNewUid, // 修改主责部门需要项目uid,
                arg_new_ou: this.refs.mainDeptRadioGroup.getData().deptname_p,
                arg_new_dept_name: mainDeptName,
            }
            let infoData = request('/microservice/allproject/project/ProjectMonitorProjectChangeMaindeptUpdateValidate',postData )
            infoData.then(data =>{
                if(data.RetCode==='1'){
                    this.setState({
                        mainDeptData:{
                            validateStatus: 'warning',
                            errorMsg: data.RetVal,
                        }
                    })
                }
            })
     ...
     * @param callback 回调函数
     */
    replaceMoneyCheck = (rule, value, callback) => {
        if (value && Number(Number(value).toFixed(2)) < Number(Number(this.props.mixReplaceMoney).toFixed(2))) {
            callback('预估投资替代额必须大于或等于' + this.props.mixReplaceMoney + '万元!');
        }
        callback();
    };
    ...
    setDeptPuDeptMgr = (i,isChange) => {
        let mainDeptInfo = {};
        if (isChange !== 'isChange' && 'dept_id' in this.props.mainProjList[i]) {
        ...
        let mgrInfo = {};
        if (isChange !== 'isChange' && 'mgr_id' in this.props.mainProjList[i]) {
        ...
        if (loadMainProj === '0') {
            ...
            for (let i = 0; i < this.props.mainProjList.length; i++) {
                if (value === this.props.mainProjList[i].proj_id) {
                    //填充表单数据
                    form.setFieldsValue(setFieldItemData(this.props.mainProjList[i],'isChange'));
                    //同时在组件外层修改主责部门、配合部门、项目经理的附加数据
                    this.setDeptPuDeptMgr(i,'isChange');
                    break;
                }
            }
        }
                                    ...
                                    <FormItem label="预估投资替代额" {...formItemLayout} >
                                        {getFieldDecorator('replace_money', {
                                            rules: [{
                                                ...
                                                validator: this.replaceMoneyCheck
                                            }
                                            ],
                            ...
                            <Row gutter={16}>
                                <Col className="gutter-row" {...FormItemCol.preCol}>
                                    <FormItem label="主建部门" {...formItemLayout}
                                              validateStatus={this.state.mainDeptData.validateStatus}
                                              help={this.state.mainDeptData.errorMsg || ''}
                                    >
                                        ...
                                        (<TextArea rows={2} style={{width: '85%'}} disabled={true}/>
                                        )}
                            ...
                            <Row gutter={16}>
                                <Col className="gutter-row" {...FormItemCol.preCol}>
                                    <FormItem
                                        label={
                                            <span>
                                                <Tooltip title={'变更归属部门后，将按照最新部门进行变更审核流程'}>
                                                    <Icon
                                                        type='exclamation-circle'
                                                        style={{color:'#ffbf00',marginRight:5}}
                                                    />
                                                </Tooltip>
                                                <span>
                                                    {'归属部门'}
 ...
 * 功能：实现tabs中基本信息页面功能
 */
class ProjChangeInfo extends React.Component {
    state = {
        puInfo: {},
        deptInfo: {},
        mgrInfo: {},
            ...
            if (!err) {
                ...
                for (let i = 0; i < pms_list.length; i++) {
                    ...
                    if (pms_list[i].opt_type === 'insert') {
                        ...
                        pms_temp.uuid = pms_list[i].uuid;       //此处与启动时不一样
    ...
    render() {
        const { mainProjList, projTypeList, projOldInfo, pms_list, projInfoLimit} = this.props;
        ...
        return (
            <div>
                <ProjInfoNew
                    ...
                    mainDeptData={this.props.mainDeptData}
