...
class MileAddModal extends React.Component {
  ...
  beginTimeCheck = (rule, value, callback)=> {
    ...
    /*if (value > form.getFieldValue('plan_end_time')) {
      callback('开始时间不能大于结束时间');
    }*/
    /* //同时验证结束时间，设置force为true
    if (value && this.state.endTimeDirty) {
      form.validateFields(['plan_end_time'], { force: true });
    }*/
    callback();
  };
  ...
  endTimeCheck = (rule, value, callback)=> {
    ...
    /*if (value < form.getFieldValue('plan_begin_time')) {
      callback('结束时间不能小于开始时间');
    }*/
    /*//同时验证开始时间，设置force为true
    if (value && this.state.beginTimeDirty) {
      form.validateFields(['plan_begin_time'], { force: true });
    }*/
    callback();
  };
  ...
  handleOk = () => {
    this.refs.mileAddModalNew.validateFields((err, values) => {
      if (err) {
        ...
        const plan_begin_time = moment(values.plan_begin_time._d).format(dateFormat);
        const plan_end_time = moment(values.plan_end_time._d).format(dateFormat);
        let mileParams = {
        ...
        const{dispatch,mileStoneList,remainWorkLoad} = this.props;
        dispatch({
          type: 'projChangeApply/mileAddModal',
          data:mileParams,
    ...
    return (
      <div>
        <Modal
          ...
          width={640}
        >
          <MileAddModalNew  ref="mileAddModalNew"
