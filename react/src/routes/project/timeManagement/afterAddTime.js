...
class AfterAddTime extends PureComponent {
  state = {
    okWindow: false, // 确认弹窗
  };
  ...
  selectChange = (v) => {
    ...
    dispatch({
      type: "afterAddTime/selectChange",
      selectProj: v
    });
  };
  ...
  CheckboxGroupChange = (value) => {
    const {dispatch} = this.props;
    dispatch({
      type: "afterAddTime/CheckboxGroupChange",
      value: value,
    });
  };
  ...
  renderColumns = (text, record, column) => {
    let { IsCommit, invalid_date, effective_date, timeSelect } = this.props;
    let begintime = JSON.parse(timeSelect).weekStart;
    let endtime = JSON.parse(timeSelect).weekEnd;
  ...
  saveAndSubmit = (saveOrSubmit) => {
    const {dispatch} = this.props;
    dispatch({
      type: "afterAddTime/saveAndSubmit",
  ...
  handleChange(value, column, record) {
    const {dispatch} = this.props;
    dispatch({
      type: "afterAddTime/handleChange",
      value: value,
      column: column,
      record: record
    });
  ...
  // 补录时间选择
  selectTimeChange = (selectValue)=> {
    const {dispatch} = this.props;
    dispatch({
      type: "afterAddTime/selectTimeChange",
      selectValue: selectValue,// selectValue 是一个json.stringfy处理过的对象
    });

  }

  render() {
    const {allProjectTime, allActivityList, selectProjectKey, totalHours, timeSelect, IsCommit, hasAfterTime,hasConditions} = this.props;
    // 当前周期的开始时间和结束时间
    let begintime;
    let endtime;
    if ( timeSelect !== '' ) {
      begintime = JSON.parse(timeSelect).weekStart;
      endtime = JSON.parse(timeSelect).weekEnd;
    }


    // 补录周期
    let timeList = this.props.timeList.map((timeItem,index) => {
      return(
        <Option value={JSON.stringify(timeItem)} key={index} style={{ fontWeight:'bold'}}>
          {timeItem.weekStart+' 至 '+timeItem.weekEnd}
        </Option>)
    })
          ...
          {hasAfterTime ?
            <div style={{paddingLeft: 32, paddingRight: 20}}>
              <div style={{background: 'white', marginTop: '15px'}}>
                ...
                <span style={{marginLeft: "91px"}}>工时总数 : </span>
              ...
              <div style={{color: '#FA7252', fontStyle: 'italic', marginTop: 32}}>
                ...
                <p>2、补录的工时只能在 {begintime + ' 至 ' + endtime} 期间提交，且所有项目只能提交一次</p>
                <p>3、补录的工时以0.5小时为一个单位</p>
              </div>
...
function mapStateToProps(state) {
  return {
    loading: state.loading.models.afterAddTime,
    ...state.afterAddTime
  }
}

export default connect(mapStateToProps)(AfterAddTime);
