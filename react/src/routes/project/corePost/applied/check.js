...
import {Table, Select, Icon, Input, Tooltip, Button, Tabs, Pagination, Row, Col, DatePicker, Modal, Breadcrumb
} from 'antd';
...
class Query extends React.PureComponent {
  ...
  changedText = (value) =>{
    ...
    dispatch({type: 'appliedCheckQuery/changedText',
  ...
  addResetPosts = () =>{
    const {dispatch} = this.props
    dispatch({
      type: 'appliedCheckQuery/addResetPosts',
      ele:this.state.ele,
      callback:(res)=>{
        if(res=='1'){
          this.setState({
        ...
        return (
            <div className={styles['pageContainer']}>
                <Breadcrumb separator=">">
                  ...
                  <Breadcrumb.Item>{'续聘审核'}</Breadcrumb.Item>
                </Breadcrumb>
                <div><p style={{textAlign: 'center', fontSize: '20px', marginBottom: '10px'}}>续聘审核</p></div>
...
function mapStateToProps(state) {
    return {
        loading: state.loading.models.appliedCheckQuery,
        ...state.appliedCheckQuery
    };
}
