...
import {Table, Select, Breadcrumb, Icon, Input, Tooltip, Button, Tabs, Pagination, Row, Col, DatePicker, Modal, Popconfirm} from 'antd';
...
class Query extends React.PureComponent {
  ...
  changedText = (value) =>{
    ...
    dispatch({type: 'resCheckQuery/changedText',
  ...
  addResetPosts = () =>{
     const {dispatch} = this.props
     dispatch({
       type: 'resCheckQuery/addResetPosts',
       ele:this.state.ele,
       callback:(res)=>{
         if(res=='1'){
           this.setState({
        ...
        return (
            <div className={styles['pageContainer']}>
                 <Breadcrumb separator=">">
                   ...
                   <Breadcrumb.Item>{'责任承诺书审核'}</Breadcrumb.Item>
                 </Breadcrumb>
                <div><p style={{textAlign: 'center', fontSize: '20px', marginBottom: '10px'}}>责任承诺书审核</p></div>
...
function mapStateToProps(state) {
    return {
        loading: state.loading.models.resCheckQuery,
        ...state.resCheckQuery
    };
}
