...
import {Table, Select, Icon, Breadcrumb, Tooltip, Button, Pagination, Row, Col, DatePicker, Input, Upload, message, Modal, Popconfirm} from 'antd';
...
class Info extends React.PureComponent {
   ...
   */
    upDataReturn = (info, elem, index) => {
      ...
      dispatch({
        type: 'responsCheckReturn/changeNum',
        num:index
      });
      this.setState({
        reload:true})
      if (info.file.status === 'done') {
        const {dispatch} = this.props;
        dispatch({
          type: 'responsCheckReturn/upDataReturn',
          corepositionId: elem,
          name: info.file.name,
   ...
   //模态窗确认
     sureAwnser = () => {
         ...
         dispatch({type: 'responsCheckReturn/resetCond',elem});
     };
        ...
        return (
            <div className={styles['pageContainer']}>
                <Breadcrumb separator=">">
                  ...
                  <Breadcrumb.Item>{'责任承诺书退回'}</Breadcrumb.Item>
                </Breadcrumb>
                <div><p style={{textAlign: 'center', fontSize: '20px', marginBottom: '10px'}}>责任承诺书申请退回</p></div>
...
function mapStateToProps(state) {
    return {
        loading: state.loading.models.responsCheckReturn,
        ...state.responsCheckReturn
    };
}
