...
class ExcelImportPromote extends Component {
  onImportExcel = file => {
    // 获取上传的文件对象
      ...
      try {
        ...
        for (const sheet in workbook.Sheets) {
          ...
          if (workbook.Sheets.hasOwnProperty(sheet)) {
            ...
            PromoteData = PromoteData.concat(XLSX.utils.sheet_to_json(workbook.Sheets[sheet]));
        ...
        dispatch({
          type:'promoteImportModel/PromoteImport',
          PromoteData,
        });
  ...
  render() {
    return (
        <Button style={{align:'right'}} className={styles['upload-wrap']} width={"100%"}>
          ...
          <span className={styles['upload-text']} >录入晋升信息</span>
        </Button>
    );
  }
}

export default ExcelImportPromote;
