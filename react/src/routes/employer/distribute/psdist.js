 ...
 * 创建日期：2017-08-20
 */
function PsDist(SearchUI) {
  return class PsDist extends React.Component {
    state = {
      activeKey:'0',
    }
    ...
    submit = async(rankList,ratio) =>{
      ...
      const subRes = await(service.psPMDistSubmit(arg_params));
      if(subRes.RetCode === '1'){
    ...
    render() {
      ...
      return <div className={Style.wrap}>
        <Tabs  type="card" defaultActiveKey={activeKey} style={{paddingTop: '180px'}}>
            ...
            <TabPane tab='项目经理' key={activeKey}>
              {rank?<ToDistList empList={projList} rankRatio = {rank[0]} reset={this.reset} loading = {loading}
                year={year} season={season} checker_info="副院长" submit={this.submit} score_has_null={score_has_null}
                />:
...
export default connect(mapStateToProps)(PsDist(SearchUI))
