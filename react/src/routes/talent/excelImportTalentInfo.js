...
class ExcelImportTalentInfo extends Component {
  onImportExcel = file => {
    // 获取上传的文件对象
      ...
      try {
        ...
        for (const sheet in workbook.Sheets) {
          ...
          if (workbook.Sheets.hasOwnProperty(sheet)) {
            ...
            talentData = talentData.concat(XLSX.utils.sheet_to_json(workbook.Sheets[sheet]));
        ...
        dispatch({
          type:'talentInfoImportModel/talentImport',
          talentData,
        });
  ...
  render() {
    return (
        <Button className={styles['upload-wrap']} width={"100%"}>
          ...
          <span className={styles['upload-text']} >录入人才信息</span>
        </Button>
    );
  }
}

export default ExcelImportTalentInfo;
