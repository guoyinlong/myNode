...
class QuarterReportDetail extends PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            modalVisible: false
    ...
    fillBlur = (indexTypeId, indexItemId, value, indexTypeIndex, indexItemIndex) => {
        ...
        const quarter = location.query.quarter;
        if (value) {
            dispatch({
    ...
    // 确认按钮
    confirmBtn = () => {
        const {indexId, quarter, tag, flowId, flowLinkId, taskUUID, taskBatchid} = this.props.location.query;
        if (tag == 2) {
            this.props.dispatch({
    ...
    render() {
        ...
        const {score, totalScore, quarter, tag} = location.query;
        ...
        return (
            <div>
                <IndexContent
                    ...
                    title={tag == 2 ? `${name}-${quarter}季度填报指标` : `${unitMap[unit]}-${indexTypeMap[name]}-${quarter}季度填报指标`}
                    isEvaluate={false}
                    showScore={false}
...
export default connect(mapStateToProps)(QuarterReportDetail);