...
export  default  class SearchUI extends React.Component{
  ...
  //需要搜索的字段
  needSearch=['funds_type','team_name','apply_username','subject_name'];
  column = [
    {
      title:'年度',
      width:'100px',
    ...
    return(
      <Spin tip="加载中..." spinning={this.props.data.loading}>
        <div>
          ...
          <div style={{marginTop:'-22px'}}><TableSearch columns = {this.column} dataSource={list} needSearch={this.needSearch} scroll={{ x: 1720 }}/></div>
        </div>
      </Spin>
    )
  }
