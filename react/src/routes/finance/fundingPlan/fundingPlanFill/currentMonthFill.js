 ...
 * 功能：本月资金计划填报组件
 */
class CurrentMonthFill extends React.PureComponent {
  state = {
  ...
  addCurrentMonthFillData = () => {
    this.props.dispatch({
      type:'fundingPlanFill/addCurrentMonthFillData'
    });
  };
  ...
  deleteCurrentMonthFillData = (index) => {
    this.props.dispatch({
      type:'fundingPlanFill/deleteCurrentMonthFillData',
      index:index
    });
  };
  ...
  editCurrentMonthCellData = (e,index,colType) => {
    ...
    this.props.dispatch({
      type:'fundingPlanFill/editCurrentMonthInputData',
      value:value,
      index:index,
      colType:colType
    });
  ...
  changeSelectValue = (value,record,colType) =>{
    ...
    if (colType === 'subject_id') {
      ...
      if (selectFeeFlag === '1' || selectFeeFlag === '2') {
        ...
        this.props.dispatch({
          type:'fundingPlanFill/editCurrentMonthSelectData',
          value:value,
          index:record.key,
          colType:colType,
  ...
  hideOfficeModal=(flag)=>{
    if(flag === 'confirm'){
      ...
      if (officeTableData.length > 0) {
        ...
        this.props.dispatch({
          type:'fundingPlanFill/editCurrentOfficeData',
  ...
  hideCapexModal=(flag)=>{
    if(flag === 'confirm'){
      ...
      if (capexTableData.length > 0) {
        ...
        this.props.dispatch({
          type:'fundingPlanFill/editCurrentCapexData',
          index:this.state.chlidRowsIndex,
  ...
  render() {
    ...
    let columns = [
      {
        ...
        render: (text, record, index) => {
          ...
          if (disabledData === false) {
            return (
              <Popconfirm title="确定删除吗?" onConfirm={() => this.deleteCurrentMonthFillData(record.key)}>
    ...
    return (
      <div>
        ...
        <br/>
        <Table
          dataSource={this.props.currentMonthFillTableData.filter(item => item.opt_type !== 'delete')}
          columns={columns}
          className={styles.fillTable}
          bordered={true}
          pagination={false}
