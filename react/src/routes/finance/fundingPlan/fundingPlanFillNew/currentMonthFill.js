...
import { Button,Tooltip,Table,Select,TreeSelect,Input,Popover,Icon,Modal } from "antd";
...
class CurrentMonthFill extends React.Component{
  constructor(props){
    super(props)
  }
  state = {};
  copyCurrentMonthFillData = () => {
    this.props.dispatch({
      type:'fundingPlanFillNew/copyCurrentMonthFillData'
    });
  };
  addCurrentMonthFillData = () => {
    this.props.dispatch({
      type:'fundingPlanFillNew/addCurrentMonthFillData'
    });
  };
  deleteCurrentMonthFillData = (index) => {
    this.props.dispatch({
      type:'fundingPlanFillNew/deleteCurrentMonthFillData',
      index:index
    });
  };
  clearCurrentMonthFillData= (index) => {
    this.props.dispatch({
      type:'fundingPlanFillNew/clearCurrentMonthFillData',
      index,
    });
  };
  changeSelectValue = (value,record,colType) =>{
    ...
    if (colType === 'subject_id') {
      ...
      if (selectFeeFlag === '1') {
        ...
        this.props.dispatch({
          type:'fundingPlanFillNew/editCurrentMonthSelectData',
          value:value,
          index:record.key,
          colType:colType,
  ...
  editCurrentMonthCellData = (e,index,colType) => {
    ...
    this.props.dispatch({
      type:'fundingPlanFillNew/editCurrentMonthInputData',
      value:value,
      index:index,
      colType:colType
    });
  };
  saveCurrentMonthFillData = (flag,tabFlag) => {
    this.props.dispatch({
  ...
  render() {
    const { fundStageData,roleType,canApplyUserList,currentMonthFillTableData,feeListTree} = this.props.data;
    ...
    //   if(currentMonthFillTableData[i].state_code === '3'){ //如果有审核通过的则不能提交
    //     canSubmit = true;
    //     break;
    //   }
    // }
    //fundStageData 资金计划填报阶段  report_type  1 预填报  2 追加 3 调整 4 其他（非填报阶段）
    //roleType 角色  report_type  1 普通员工  2 小组管理员 3 团队管理员 4 小组/团队管理员
    //canApplyUserList 可申报人名单 他购时获取

    //currentMonthFillTableData 最重要的数据  查询出来的表格里面的数据
    // 添加一条：
    //删除一条：库里已经有的删除，新加的删除
    //修改一条：
    // state_code : 1 保存  2 待审核  3 审核通过  4 审核退回

    //feeListTree 科目对应的选择树  capex时 flag 为3 通过flag判断到底弹不弹框
              ...
              if(record.boundModalFlag === '1'){
                ...
                return (
                  <Input
                    ...
                    disabled={false}
                    maxLength='12'
                    onChange={(e)=>this.editCurrentMonthCellData(e,record.key,'funds_plan')}
                  />
                );
              }
            }
            ...
            if (disabledData === true) {
              ...
              return (
                <Input
                  ...
                  disabled={false}
                  maxLength='100'
                  onChange={(e)=>this.editCurrentMonthCellData(e,record.key,'remark')}
                />
              );
            }
          },
        },
        {
          ...
          render: (text, record) => {
            ...
            if (disabledData === false) {
              return (
                <a onClick={() => this.deleteCurrentMonthFillData(record.key)}>{'删除'}</a>
              );
            }else{
              return (
            ...
            if (disabledData === true) {
              ...
              return (
                <Input
                  ...
                  disabled={false}
                  maxLength='12'
                  onChange={(e)=>this.editCurrentMonthCellData(e,record.key,'funds_current_amount')}
                />
              );
            }
          },
          ...
          render: (text, record) => {
            ...
            if (disabledData === false) {
              if( record.fill_state_code === '3'){  //预填报且审核通过
                return (
                  <a onClick={() => this.clearCurrentMonthFillData(record.key)}>{'清空'}</a>
                );
              }else{
                return (
                  <a onClick={() => this.deleteCurrentMonthFillData(record.key)}>{'删除'}</a>
    ...
    return (
      <div>
            ...
            {fundStageData.report_type === '3'?
              ...
              <span>
                {roleType ==='2' && flag ==='append'?
                  <Button onClick={this.addCurrentMonthFillData} type='primary'>新增</Button>
                  :
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;
        {
          ...
          <span>
            {fundStageData.report_type === '3'?
              ...
              <span>
                {roleType ==='2' && flag ==='append'?
                  <Button onClick={this.copyCurrentMonthFillData} type='primary'>{COPE_DATA}</Button>
                  :
        ...
        <div style={{margin:'10px 0',position: 'relative'}}>
          ...
          <Table
            dataSource={currentMonthFillTableData.filter(item => item.opt_type !== 'delete')}
            columns={columns}
            className={styles.fillTable}
            bordered={true}
            pagination={false}
          ...
          <span>
            {
              ...
              <span>
                {
                  roleType ==='2'?
                  <Button onClick={()=>this.saveCurrentMonthFillData('1','2')} type='primary'>保存</Button>
                  :
                  <Tooltip title={'该阶段只能小组管理员在调整模块保存！'}>
                    <Button type='primary' disabled={true}>{'保存'}</Button>
                  </Tooltip>
                }
              </span>
          ...
          <span>
            {
              ...
              <span>
                {roleType ==='2'?
                  <Button onClick={()=>this.saveCurrentMonthFillData('2','2')} type='primary'>提交</Button>
                  :
                  <Tooltip title={'该阶段只能小组管理员在调整模块提交！'}>
                    <Button type='primary' disabled={true}>{'提交'}</Button>
                  </Tooltip>
                }
              </span>
        ...
        <CapexFillAdd fundStageData={this.props.data.fundStageData} ref='capexFillAdd' dispatch={this.props.dispatch} flag={flag} isPriorYear='1'/>
      </div>
    );
  }
}
export default CurrentMonthFill;
