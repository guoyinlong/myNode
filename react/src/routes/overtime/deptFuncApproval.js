...
class DeptFuncApproval extends Component{
  constructor (props) {
    super(props);
    this.state = {
      post_name:"",
...
//选择不同意，显示驳回意见信息
  ...
  handleOk = () => {
    ...
    const{dispatch} = this.props;
    if(this.props.deptInfoRecords.task_name==='部门加班申请'){
  ...
  render() {
    ...
    console.log("approvalNowList:"+approvalNowList);
    ...
    return (
      <div>
        ...
        <Card title="加班申请汇总"  className={styles.card} extra={<a icon="download" onClick={this.expExl} disabled="" style={{display:deptInfoRecords.step==="人力资源专员归档"?"":"none"}}>导出数据</a>}>
          <Table
        ...
        {
          deptInfoRecords.task_name === "部门加班统计"
            ?
...
DeptFuncApproval = Form.create()(DeptFuncApproval);
export default connect(mapStateToProps)(DeptFuncApproval);
