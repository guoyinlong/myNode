...
class DeptStatsApproval extends Component{
  constructor (props) {
    super(props);
    this.state = {
      post_name:"",
...
//选择不同意，显示驳回意见信息
  ...
  gotoTeamDetails = (record) =>{
    ...
    dispatch({
      type:'approval_model/queryTeamStatsList',
      orig_proc_task_id,
    });
  ...
  team_columns2 = [
    ...
    { title: '操作', dataIndex: '',  key: 'x', render: (text,record,index) => (
        <span>
        {
          record.file_relativepath !== ''&&record.file_relativepath !== null ?
              <span>
  ...
  handleOk = () => {
    ...
    return new Promise((resolve) => {
      dispatch({
        type:'approval_model/submitDeptStatsApproval',
        approval_if,
        approval_advice,
        orig_proc_inst_id,
        orig_proc_task_id,
  ...
  submitcheck = () =>{
    ...
    if (approval_if=='不同意'){
      ...
      if (this.state.if_end_task === '1'){
        ...
        return new Promise((resolve) => {
          dispatch({
            type:'approval_model/submitDeptStatsApprovalEnd',
            approval_if,
            approval_advice,
            orig_proc_inst_id,
            orig_proc_task_id,
  ...
  exportExcel=()=>{
    ...
    if(personDataListExport.length !== 0){
      ...
      message.info("无"+deptName+"加班统计数据");
    }
  };
          ...
          <Form style={{marginTop: 10}}>
            ...
            <FormItem label="节假日" {...formItemLayout}>
              {getFieldDecorator('holidays', {
                ...
                <Select style={inputstyle}  size="large" style={{width: 200}} placeholder="劳动节" disabled={true}>
            ...
            <FormItem label="申请日期" {...formItemLayout}>
              {getFieldDecorator('timeApply', {
                initialValue: deptInfoRecords.create_time
              })(<span> {this.getCurrentDate()} </span>)}
            </FormItem>
          </Form>
        </Card>
        <Card title="加班统计汇总" className={styles.card} extra={<a icon="download" onClick={this.exportExcel} disabled="" style={{display:deptInfoRecords.step==="人力资源专员归档"?"":"none"}}>一键导出</a>}>
            <Table
        ...
        <Card title={<div style={{textAlign: "left"}}>审批意见</div>} className={styles.card} bordered={true} style={{display: ''}}>
          ...
          <FormItem label="团队负责人审核">
            <TextArea
              style={inputstyle}
              style={{ minHeight: 32 }}
              placeholder={this.state.teamsum}
              autosize={{ maxRows: 10 }}
              disabled={true}
            />
          </FormItem>
...
DeptStatsApproval = Form.create()(DeptStatsApproval);
export default connect(mapStateToProps)(DeptStatsApproval);
