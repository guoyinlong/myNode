...
import { Button, Table, Select, Modal, message, Form, Row, Card, Col, Input, DatePicker } from "antd";
...
class absence_approval extends Component {
  ...
  handleOk = () => {
    ...
    return new Promise((resolve) => {
      dispatch({
        type: 'absence_approval_model/affairApprovalSubmit',
        approval_if,
        approval_advice,
        orig_proc_inst_id,
        orig_proc_task_id,
  ...
  submitcheck = () => {
    ...
    if (approval_if == '不同意') {
      ...
      if (if_end_task == '1') {
        ...
        return new Promise((resolve) => {
          dispatch({
            type: 'absence_approval_model/affairApprovalEnd',
            approval_if,
            approval_advice,
            orig_proc_inst_id,
            orig_proc_task_id,
  ...
  breakOffInfo = [
    ...
    { title: '请假人姓名', dataIndex: 'absence_user_name' },
    { title: '请假人员工编号', dataIndex: 'absence_user_id' },
  ...
  render() {
    ...
    let personInfo = {};
    if (personsList.length > 0 && personsList[0]) {
    ...
    return (
      <div>
        <Row span={1} style={{ textAlign: 'center' }}>
          <h2>中国联通济南软件研究院员工事假申请审批表</h2></Row>
          ...
          <Card title="基本信息" className={styles.r}>
            <Form style={{ marginTop: 10 }}>
              ...
              <FormItem label="请假类型" {...formItemLayout}>
                <Input style={inputstyle} value={personInfo.absence_type_name} disabled={true} />
              </FormItem>
            </Form>
          </Card>
            ...
            <Form>
              <FormItem label={'实际请假天数'} {...formItemLayout}>
                {getFieldDecorator('rel_absence_days', {
                  ...
                  rules: [{ pattern: /^([1-9][0-9]*)?$/, required: true, message: '请填写请假天数' },],
                })(
                  <Input placeholder="请假天数" style={{ width: '150', color: '#000' }} name="absence_days" disabled={ifdisplay} />
                )}
              </FormItem>
              {
                this.state.now_post_name == "申请人销假"
                  ?
                  <FormItem label={'实际请假日期'} {...formItemLayout}>
                    ...
                    {getFieldDecorator('rel_end_date', {
                      initialValue: '',
                      rules: [{ required: true, message: '请选择请假结束日期' },],
                    })(
                      <DatePicker
                        placeholder="结束日期"
                        style={{ width: '150' }}
