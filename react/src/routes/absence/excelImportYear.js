...
class ExcelImportYear extends Component {
  onImportExcel = file => {
    // 获取上传的文件对象
      ...
      try {
        ...
        for (const sheet in workbook.Sheets) {
          ...
          if (workbook.Sheets.hasOwnProperty(sheet)) {
            ...
            YearData = YearData.concat(XLSX.utils.sheet_to_json(workbook.Sheets[sheet]));
        ...
        dispatch({
          type: 'year_person_info_model/yearImportData',
          YearData,
        });
  ...
  render() {
    return (
      <Button className={styles['upload-wrap']} width={"100%"}>
        ...
        <span className={styles['upload-text']} >导入年假信息</span>
      </Button>
    );
  }
}

export default ExcelImportYear;
