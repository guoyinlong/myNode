...
import { Table, Input, Button, Popconfirm, Form, Modal, message, Breadcrumb, Spin } from 'antd';
import styles from './budgetChangeModify.less';

const TextArea = Input.TextArea;
class budgetChangeModify extends React.PureComponent {
  ...
  columns = (deptCountArray, budgetArray, tableId, projId, changeFlag) => {
  ...
  render() {
    ...
    if(projTravelBudgetList.length !==0 ) {
      ...
      projTravelBudgetList.map((data, index) => {
        ...
        const columns = this.columns(rowSpanArray, budget, index, projInfo.projId, projInfo.projCanChange);
    ...
    if(projInfo.projCanChange === '0') {
      statusLabel = '审核中', btnDiv = null;
    }else {
      // 超过部门预算的情况
    ...
    const task_info = JSON.parse(this.props.location.query.arg_task_info);
    const preQuery = {
      arg_check_id: this.props.location.query.arg_check_id,
      arg_proj_id: projInfo.projId,
      arg_proj_uid: projInfo.projUid,
      arg_task_batchid: task_info.arg_task_batchid,
      arg_task_uuid: task_info.arg_task_uuid,
      arg_task_wf_batchid: task_info.arg_task_wf_batchid,
    }
      ...
      <div className={styles.container}>
        <Spin spinning={this.props.loading}>
          <Breadcrumb separator=">">
            ...
            <Breadcrumb.Item><Link to={{pathname:"/travelBudgetChangeReturn", query: preQuery}}>任务详情</Link></Breadcrumb.Item>
            <Breadcrumb.Item>退回修改</Breadcrumb.Item>
          </Breadcrumb>
...
export default connect(mapStateToProps)(budgetChangeModify);
