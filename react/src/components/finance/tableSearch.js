...
import styles from '../employer/employer.less'
...
export  default  class TableSearch extends React.Component{
  ...
  render(){
    ...
    dataSource && dataSource.length!==0 && dataSource.forEach((i,index)=>i.key =index);
    ...
    return(
      <div>
        {
          needSearch.length !== 0?
            <div style={{textAlign:'right'}}><Button type='primary' disabled={isDisable} onClick={this.clearFilter} style={{marginBottom:'10px'}}>清空条件 </Button></div>
            :
            null
        }
        <Table
