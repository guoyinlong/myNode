...
export default class Capacity extends React.PureComponent {
  ...
  setCapacityOption = (data) => {
    return {
      ...
      grid: {
        y: 40,
        y2: 25,
        bottom:'38%',  //x轴的文字如果太长会受到遮挡，显示不签，可以用grid属性解决
      },
      yAxis: [
        {
          type: 'value'
  ...
  render() {
    let postData = {
      ...
      'arg_stat_type': 'n', //n表示查询统计时长，c为表示查询次数统计，不可为其它值
      'arg_stat_scope': 'd' //r 表示查询会议室统计，为 d 表示查询部门统计
  };
    ...
    queryData.then(capacityData => {
      if (capacityData.RetCode === '1') {
        ...
        for(let i = 0; i < data.xAxis.length; i++){
          if(data.xAxis[i].indexOf('-') != -1){
              this.state.xAxis.push(data.xAxis[i].split('-')[1]);
          }
        }
